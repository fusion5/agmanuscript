  ...
  where
    ...
    entryTag (key, "main")
      = void $ C.mapOutput (Entry key)
          $ XML.many' $ XML.tagIgnoreAttrs "sense"
...
contentRec = T.concat <$> XML.many'
  (XML.tagIgnoreAttrs XML.anyName contentRec `XML.orE` XML.contentMaybe)

selectDepth :: Conduit XMLStream.Event Entry ()
selectDepth =
  void $ XML.tagIgnoreAttrs "TEI.2"
    $ XML.many' $ XML.tagIgnoreAttrs "text"
      $ XML.many' $ XML.tagIgnoreAttrs "body"
        $ XML.many' $ XML.tagIgnoreAttrs "div0"
          $ XML.many' parseEntry
